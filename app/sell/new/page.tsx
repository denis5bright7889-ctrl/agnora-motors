...
// Login-free public listing form. Anyone can post a car — no account needed.
// We collect the seller's name + phone so buyers have a way to reach them.
  ...
  if (saved) {
    return (
      <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <p className="font-display text-2xl font-medium">Your car is live!</p>
    ...
    <div className="min-h-[calc(100vh-4rem)] px-4 py-10">
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-display text-3xl font-medium">List your car</h1>
            <p className="text-sm text-muted mt-0.5">
              No account needed — fill in the details and publish for free.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Section title="Your contact details" description="Buyers will use this to reach you about the car.">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Phone number" error={errors.sellerPhone?.message}>
                <input
                  ...
                  className={inputCls(!!errors.sellerPhone)}
                />
              </Field>
            </div>
          </Section>
            ...
            {features.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {features.map((f) => (
                  ...
                  >
                    {f}
                    <button type="button" aria-label={`Remove ${f}`} onClick={() => toggleFeature(f)}>
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ))}
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
            <button
              ...
              className="w-full sm:flex-1 h-12 rounded-full bg-accent text-white text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
            ...
            <Link
              href="/sell"
              className="w-full sm:w-auto h-12 rounded-full border border-border px-6 text-sm font-medium hover:bg-surface-2 transition-colors flex items-center justify-center"
            >
              Cancel
            </Link>
          </div>
          ...
          <p className="text-xs text-muted text-center">
            Free to list · no account required · your details are only shared with interested buyers.
          </p>
        </form>
      </div>
    </div>
